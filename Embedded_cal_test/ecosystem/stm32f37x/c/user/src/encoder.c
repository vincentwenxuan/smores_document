...
void initEncoder(){
  ...
  // QEP Timer Enable
  #if ENCODER_QEP_TIMER == 15 || ENCODER_QEP_TIMER == 19
    RCC_APB2PeriphClockCmd(MAKENAME(RCC_APB1Periph_TIM,ENCODER_QEP_TIMER), ENABLE);
  #else
...
float encoderReadRad() {
  ...
  float angle = ticks * ENCODER_TICKS_PER_REV * 2.0f * PI;
...
float compute_speed_fr(float frequency, float motor_speed_smoothing_factor)
{
  ...
  float temp_motor_speed = ((float)encoder_position_diff)*frequency*2.0f*PI*ENCODER_TICKS_PER_REV; // speed in rad/s
  #ifdef ENCODER_REVERSE_DIRECTION
