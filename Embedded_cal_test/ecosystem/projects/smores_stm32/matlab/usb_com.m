classdef usb_com < handle
    ...
    properties (Constant)
        ...
        MsgMagnet = struct('Type', 109, 'Fields', struct('face', 'uint8', 'state', 'uint8'));
        MsgModule = struct('Type', 110, 'Fields', struct('data', 'uint16'));
        MsgTorque = struct('Type', 111, 'Fields', struct('DoF', 'uint8', 'direction', 'uint8', 'speed', 'uint8'));
    end
    ...
    methods
        ...
        function send_usb_torque(UsbCom, Dof)
            msg.DoF = Dof.DoF;
            msg.direction = Dof.direction;
            msg.speed = Dof.speed;
            UsbCom.com.SendMsg(UsbCom.MsgTorque, msg);
        end
