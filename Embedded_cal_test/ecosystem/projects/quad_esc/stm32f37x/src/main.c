...
#define X_CONFIG //comment if IMU is aligned with a propeller (t config)
...
static void calcControl(float des_thrust, float des_yaw, float des_pitch, float des_roll)
{
  ...
  #else // X_CONFIG
    pwm[0] = pwm_base - d_omega_y - d_omega_x + d_omega_z;
    pwm[1] = pwm_base + d_omega_x - d_omega_y - d_omega_z;
    pwm[2] = pwm_base + d_omega_y + d_omega_x + d_omega_z;
    pwm[3] = pwm_base - d_omega_x + d_omega_y - d_omega_z;
  #endif
