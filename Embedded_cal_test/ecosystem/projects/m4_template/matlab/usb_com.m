classdef usb_com < handle
    ...
    properties (Constant)
        MsgMyDataSpec = struct('Type', 101, 'Fields', struct('cmd', 'uint16'));
        MsgMyData = struct('Type', 102, 'Fields', struct('data', 'uint16'));
    end
    ...
    methods
        ...
        function data = receive_usb_data(UsbCom)
            if WaitForMessage(UsbCom, UsbCom.MsgMyData.Type)
                data = UsbCom.data;
            end
        end
        ...
        function data = get_usb_data(UsbCom, cmd) % get the data from usb device
            ...
            UsbCom.com.SendMsg(UsbCom.MsgMyDataSpec, msg);
            if WaitForMessage(UsbCom,UsbCom.MsgMyData.Type) % if we got a reply
...
%             UsbCom.com.SendMsg(UsbCom.MsgMyDataSpec, msg);
        ...
        function [msg_id] = ParseMessage(UsbCom, msg_id, data)
            ...
            if msg_id == UsbCom.MsgMyData.Type %MsgMyData
                msg = UnpackMsg(UsbCom.MsgMyData,data);
