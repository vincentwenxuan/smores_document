...
int main(void)
{
  ...
  const int16_t imu_offset[6] = {0, 0, 0, 0, 0, 0};
  const float gyro_sensitivity = 65.5*180/pi;
  const float alpha = 0.001; //0.001;
  ...
  while(1)
  {
      ...
      if(rx_len > 2)
      {
        if(rx_buf[rx_len-1] & 0x80) // Valid CRC
        {
          ...
          if((rx_buf[0] == TYPE_TRPY_CMD) && (data_len == sizeof(struct TRPY_CMD))) // TRPY command
          {
            ...
            rx_quadID = trpy_cmd->quad_id*1e-4f;
    ...
    if(imu_data_buf[0] >= 16384)
      acc_pitch = -pi/2;
    else if(imu_data_buf[0] <= -16384)
      acc_pitch = pi/2;
    else
      acc_pitch  = asinf(-imu_data_buf[0]/16384.0f);
    ...
    float acc_x      = imu_data_buf[0]/16384.0f;
    float acc_y      = imu_data_buf[1]/16384.0f;
    float acc_z      = imu_data_buf[2]/16384.0f;
    ...
    if(fabsf(roll) > pi/2 || fabsf(pitch) > pi/2 )
    {
      updatePWM(0,0,0,0);
      mOrangeOFF;
      motors_on = 0;
