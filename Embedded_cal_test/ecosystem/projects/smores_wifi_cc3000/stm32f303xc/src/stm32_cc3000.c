...
void cc3000_startup_check() {
  ...
  debug_write("major is 0x");
  debug_write_u8(cc3000MajorFirmwareVersion, 16);
  debug_write(", ");
  debug_write("minor is 0x");
  debug_write_u8(cc3000MinorFirmwareVersion, 16);
  debug_write_line("\r");
  if (cc3000MajorFirmwareVersion != 0x01 || cc3000MinorFirmwareVersion != 0x20) {
    debug_write_line("Wrong firmware version!\r");
  //  while (1);
  }
  ...
  debug_write_line("Deleting old connection profiles\r");
  if (cc3000_delete_profiles() != 0) {
    debug_write_line("Failed!\r");
    while (1);
  }
  ...
  if (netapp_dhcp(&aucIP, &aucSubnetMask, &aucDefaultGateway, &aucDNSServer) != 0) {
    debug_write_line("netapp_dhcp Failed!\r");
    while (1);
  }
  ...
  if (cc3000_connect_to_ap(WLAN_SSID, WLAN_PASS, WLAN_SECURITY) != 0) {
    debug_write_line("Connect Failed!\r");
    while (1);
  }
  ...
  debug_write_line("Request DHCP\r");
...
int cc3000_setup(int patchesAvailableAtHost, int useSmartConfigData) {
  debug_write_line("cc3000_setup\r");
  cc3000_connected = 0;
  cc3000_dhcp = 0;
  ...
  if (wlan_set_event_mask(
    ...
    debug_write_line("WLAN Set Event Mask FAIL\r");
    return 1;
  }
  ...
  if (useSmartConfigData) {
    ...
    while (!cc3000_connected) {
      ...
      if (timeout > WLAN_CONNECT_TIMEOUT_MS) {
        debug_write_line("Timed out using SmartConfig data\r");
      }
      return 2;
    }
    timeout += 10;
  ...
  debug_write_line("END cc3000_setup\r");
  return 0;
}
...
//**************************************************************************************
void cc3000_setup_vbat_en() {
  debug_write_line("cc3000_setup_vbat_en\r");
...
//**************************************************************************************
void cc3000_setup_irq() {
  debug_write_line("cc3000_setup_irq\r");
...
//***************************************************************************************************************
void cc3000_setup_spi() {
  debug_write_line("cc3000_setup_spi\r");
  GPIO_InitTypeDef gpioConfig;
...
int cc3000_get_firmware_version(uint8_t *major, uint8_t *minor) {
  ...
  debug_write("cc3000_get_firmware_version: ");
  if (nvmem_read_sp_version(fwpReturn) != 0) {
    debug_write_line("Unable to read the firmware version\r");
    return 1;
  }
...
void cc3000_spi_trigger_rx_processing() {
  ...
  if (cc3000_spi_info.rx_packet[CC3000_RX_BUFFER_SIZE - 1] != CC3000_BUFFER_MAGIC_NUMBER) {
    ...
    debug_write_line("CC3000: ERROR - magic number missing!\r");
    while (1);
  }
...
void SpiOpen(spi_rx_handler_t rx_handler) {
  ...
  debug_write_line("END SpiOpen\r");
}
...
//**************************************************************************************
void SpiClose() {
  debug_write_line("TODO SpiClose\r"); // TODO SpiClose
}
...
long SpiWrite(unsigned char *pUserBuffer, unsigned short usLength) {
  ...
  if (wlan_tx_buffer[CC3000_TX_BUFFER_SIZE - 1] != CC3000_BUFFER_MAGIC_NUMBER) {
    debug_write_line("CC3000: Error - No magic number found in SpiWrite\r");
    while (1);
  }
...
int cc3000_spi_first_write(unsigned char *ucBuf, unsigned short usLength) {
  ...
  DelayMilliseconds(1); // TODO: 1ms really?
...
int cc3000_get_mac_address(uint8_t *addr) {
  debug_write_line("cc3000_get_mac_address\r");
...
int cc3000_delete_profiles() {
  if (wlan_ioctl_set_connection_policy(0, 0, 0) != 0) {
    debug_write_line("Failed to set up connection policy\r");
    return 1;
  }

  if (wlan_ioctl_del_profile(255) != 0) {
    debug_write_line("Failed deleting profiles\r");
    return 2;
  }
...
int cc3000_connect_to_ap(const char *ssid, const char *key, uint8_t secmode) {
  ...
  do {
    ...
    // Setup a 4 second SSID scanning
    cc3000_scan_ssids(4000);

    // Wait for results
    DelayMilliseconds(4500);
    ...
    debug_write_line("Waiting to connect...\r");
...
int cc3000_scan_ssids(uint32_t time) {
  ...
  if (time) {
    debug_write_line("Started AP/SSID scan\r");
  }
...
#ifndef CC3000_TINY_DRIVER
  if (wlan_ioctl_set_connection_policy(0, 0, 0) != 0) {
    debug_write_line("Failed to set connection policy\r");
    return 1;
  }
  DelayMilliseconds(500);
  if (wlan_connect(WLAN_SEC_UNSEC, (char*) ssid, strlen(ssid), 0, NULL, 0) != 0) {
    debug_write_line("SSID connection failed\r");
    return 2;
  }
  return 0;
...
int cc3000_connect_secure(const char *ssid, const char *key, int32_t secMode) {
  if ((secMode < 0) || (secMode > 3)) {
    debug_write_line("Security mode must be between 0 and 3\r");
    return 1;
  }
  ...
  if (wlan_ioctl_set_connection_policy(0, 0, 0) != 0) {
    debug_write_line("Failed setting the connection policy\r");
    return 4;
  }
...
int available(int16_t socket) {
  ...
  timeout.tv_sec = 0;
  timeout.tv_usec = 10000; // 5 millisec
...
void cc3000_display_mac_address() {
  ...
  if (cc3000_get_mac_address(macAddress) != 0) {
    debug_write_line("Unable to retrieve MAC Address!\r");
  } else {
...
int cc3000_display_ipconfig() {
  ...
  if (ipconfig.aucIP[3] == 0) {
    debug_write_line("no valid IP addresses\r");
    return 1;
  }
